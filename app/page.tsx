...
const services = [
  { icon: Layers, label: "Industrial Product Design", href: "/services/industrial-design" },
  { icon: Box, label: "2D / 3D CAD Modelling", href: "/services/cad-modelling" },
  { icon: Cpu, label: "Product Engineering", href: "/services/product-engineering" },
  { icon: Settings, label: "Prototyping", href: "/services/prototyping" },
  { icon: Settings, label: "Tool & Die Design", href: "/services/tool-die-design" },
  { icon: RotateCcw, label: "Reverse Engineering", href: "/services/reverse-engineering" },
    ...
    <>
      ...
      <section className="relative h-[90vh] min-h-[560px] overflow-hidden">
              ...
              <div key={i} className={`transition-all duration-700 ${i === current ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 absolute"}`}>
                {i === current && (
                  <div className="max-w-[700px]">
                    <div className="inline-flex items-center gap-2 border border-white/20 text-white/70 text-[0.65rem] tracking-[0.22em] uppercase px-4 py-2 rounded-full mb-8">
                      {s.tag}
                    </div>
                    <h1 className="font-display text-[clamp(2.4rem,5vw,4rem)] font-700 leading-[1.1] text-white mb-6">
                      {s.title}
                    </h1>
                    <p className="text-white/65 text-[0.97rem] leading-relaxed max-w-[500px] mb-10">
                      {s.sub}
                    </p>
        ...
        <div className="absolute bottom-8 left-6 lg:left-10 z-20 flex items-center gap-4">
          <button onClick={prev} className="w-9 h-9 rounded-full border border-white/30 hover:border-white text-white/70 hover:text-white flex items-center justify-center transition-all">
            <ChevronLeft size={16} />
          </button>
          <div className="flex gap-2">
            {slides.map((_, i) => (
              <button key={i} onClick={() => setCurrent(i)}
                className={`h-[3px] rounded-full transition-all duration-300 ${i === current ? "w-8 bg-white" : "w-3 bg-white/30"}`} />
            ))}
          </div>
          <button onClick={next} className="w-9 h-9 rounded-full border border-white/30 hover:border-white text-white/70 hover:text-white flex items-center justify-center transition-all">
      ...
      <section className="bg-white py-20 px-6 lg:px-10">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
          <div>
            ...
            <h2 className="font-display text-[clamp(1.8rem,3vw,2.8rem)] font-700 text-[#1a2535] leading-tight mb-5">
              One Studio.<br />Every Stage of Your Product Journey.
            </h2>
            <p className="text-[#5a6880] text-[0.92rem] leading-relaxed mb-4">
              Enrachna Design Labs is a full-spectrum product design and engineering studio built to eliminate the fragmentation companies face when juggling multiple vendors.
            </p>
            <p className="text-[#5a6880] text-[0.92rem] leading-relaxed mb-8">
              We are a team of designers, innovators, engineers and strategists with a common goal of making a meaningful &amp; positive impact — fueled by passion, commitment, and complete transparency.
            </p>
            <Link href="/about" className="inline-flex items-center gap-2 bg-[#111111] hover:bg-[#333] text-white px-7 py-3 text-[0.78rem] font-600 transition-colors rounded-sm">
      ...
      <section className="bg-[#f4f6f9] py-20 px-6 lg:px-10">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-[#5a6880] text-[0.88rem] leading-relaxed max-w-[640px] mb-10">
            Our comprehensive process is structured around three fundamental principles — proven instrumental in delivering successful projects tailored to customer needs.
          </p>
          <div className="flex flex-wrap items-center gap-6 mb-8">
            ...
            <Link href="/about" className="ml-auto px-6 py-3 bg-[#111111] text-white text-[0.78rem] font-600 hover:bg-[#333] transition-colors rounded-sm">
              Read More
            </Link>
          </div>
        </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {services.map((s) => (
              <Link key={s.href} href={s.href}
                className="group bg-[#f0f1f3] hover:bg-[#e8eaf0] rounded-2xl p-8 transition-all duration-200 flex flex-col items-center text-center border border-transparent hover:border-[#1a2535]/15 hover:shadow-sm">
      ...
      <section className="py-24 px-6 lg:px-10 bg-[#f4f6f9]">
        <div className="max-w-7xl mx-auto">
          <p className="text-[0.65rem] tracking-[0.25em] uppercase text-[#888] font-600 mb-3">Our Work</p>
          <div className="flex items-end justify-between flex-wrap gap-4 mb-12">
            <h2 className="font-display text-[clamp(1.8rem,3vw,2.6rem)] font-700 text-[#1a2535] leading-tight">
              Projects That Speak<br />for Themselves
            </h2>
            <Link href="/case-studies/project-alpha" className="text-[#1a2535] text-[0.78rem] font-600 hover:underline flex items-center gap-1">
              View All Work <ArrowRight size={14} />
            </Link>
          </div>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { tag: "Industrial Design · Prototyping", title: "Project Alpha", desc: "Full-cycle product design from concept sketch to production-ready prototype.", href: "/case-studies/project-alpha" },
              { tag: "Product Engineering · Tooling", title: "Project Beta", desc: "Precision tooling redesign that slashed rejection rates for a manufacturer.", href: "/case-studies/project-beta" },
              { tag: "Reverse Engineering · CAD", title: "Project Gamma", desc: "Legacy component recreated and improved with modern geometry and tolerances.", href: "/case-studies/project-gamma" },
            ].map((c) => (
              <Link key={c.href} href={c.href}
                ...
                <div className="p-6">
                  <p className="text-[0.6rem] tracking-[0.2em] uppercase text-[#888] font-600 mb-2">{c.tag}</p>
                  <h3 className="font-display text-[1.05rem] font-700 text-[#1a2535] mb-2 group-hover:text-[#111111] transition-colors">Case Study — {c.title}</h3>
                  <p className="text-[#5a6880] text-[0.8rem] leading-relaxed">{c.desc}</p>
                  <span className="inline-flex items-center gap-1 mt-4 text-[0.72rem] font-600 text-[#1a2535]">
      ...
      <section className="py-24 px-6 lg:px-10 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <h2 className="font-display text-[clamp(1.8rem,3vw,2.6rem)] font-700 text-[#1a2535] leading-tight mb-12">
            The Integrated Advantage
          </h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {whyPoints.map((w, i) => (
              <div key={i} className="p-7 bg-[#f4f6f9] rounded-xl hover:bg-[#ebebeb] transition-colors border border-[#e8e8e8]">
      ...
      <section className="bg-[#111111] py-24 px-6 lg:px-10">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div>
            ...
            <p className="text-white/55 text-[0.88rem] leading-relaxed mb-8">
              ...
              <div className="flex flex-col gap-5">
                <div className="grid grid-cols-2 gap-5">
                  <input type="text" placeholder="First Name"
                    className="w-full border-b border-white/25 focus:border-white text-white placeholder-white/35 py-3 text-[0.88rem] outline-none transition-colors bg-transparent" />
                ...
                <textarea placeholder="Message" rows={4}
                  className="w-full border-b border-white/25 focus:border-white text-white placeholder-white/35 py-3 text-[0.88rem] outline-none resize-none transition-colors bg-transparent" />
                <button onClick={() => setSent(true)}
                  className="mt-1 px-8 py-3.5 bg-white text-[#111111] text-[0.82rem] font-700 hover:bg-[#f0f0f0] transition-colors rounded-sm w-fit">
                  Submit
                </button>
              </div>
            )}
